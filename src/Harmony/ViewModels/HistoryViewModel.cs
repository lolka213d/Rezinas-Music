...

{
    ...
    public HistoryViewModel(IHistoryService history, PlayerViewModel player)
...

    {
        ...
        var buckets = new (string Title, Func<HistoryItem, bool> Match)[]

        {

            ("Сегодня", i => i.PlayedAt.Date == today),

            ("Вчера", i => i.PlayedAt.Date == yesterday),

            ("На этой неделе", i => i.PlayedAt.Date < yesterday && i.PlayedAt.Date >= weekStart),

            ("Ранее", i => i.PlayedAt.Date < weekStart)

        };



        foreach (var (title, match) in buckets)

        {
            ...
            var group = new HistoryGroup { Title = title };
    ...
    private async Task Play(HistoryItem item)

    {

        var tracks = Items.Select(i => i.Track).ToList();

        await _player.PlayQueueAsync(tracks, item.Track);
    ...
    private async Task Clear()
    {
        ...
        if (owner != null && !DarkConfirmDialog.Ask(owner, "Очистить всю историю прослушивания?"))
            return;
